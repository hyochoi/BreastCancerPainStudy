...
compare_means(Pain ~ Occupation,data=final.data)
```
...
```{r,echo=FALSE,eval=FALSE}
xlist = c("Poverty.f","marital.status.f","Primary.Payer.f2","Usual.Occupation.f2","race.f","stage.f","age.f","chemo.f","surgery.f")
...
tmp <-
  cdmd.pvt %>%
  filter(marital.status.f2 %in% c("Married","Single")) %>%
  select("Pain","marital.status.f2")
colnames(tmp) = c("Pain",x)
class(tmp[,2])
base <- ggboxplot(tmp, x=x, y="Pain",
                  color = x, palette="jco",legend="none") +
...
tmp <-
  cdmd.pvt %>%
  filter(Primary.Payer.f2 %in% c("Insurance","Medicaid")) %>%
  select("Pain","Primary.Payer.f2")
...
tmp <-
  cdmd.pvt %>%
  filter(Usual.Occupation.f2 %in% c("Employed","Retired","Unemployed")) %>%
  select("Pain","Usual.Occupation.f2")
